...
// display the line editor on a canvas
public class DView extends StackPane implements Subscriber {
    ...
    private void drawItem(Groupable g){
        if(g.hasChildren()){
            ...
            // draw selected line and its endpoints
            if (iModel.getSelectedGroups().contains(g)){
                gc.setStroke(Color.PINK);
                gc.setLineWidth(2);
                gc.strokeLine(line.getX1(), line.getY1(), line.getX2(), line.getY2());
                gc.setFill(Color.GREEN);
                gc.fillOval(line.getX1()-4, line.getY1()-4, 8, 8);          //startpoint with radius 4
                gc.fillOval(line.getX2()-4, line.getY2()-4, 8, 8);          //endpoint with radius 4
            }else {
